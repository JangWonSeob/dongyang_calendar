      ...
      <tbody>
        <tr v-for="(dayList, i) in calendarDayList">
          <td
            ...
            :class="{ today: now.month === day[0] && now.day === day[1] }"
          >
            <div class="inner">
              <span
                class="date dimmed"
                v-if="i === 0 && idx < beforeMonthCount"
                >{{ day[1] }}</span
              >
              <span
                class="date dimmed"
                v-else-if="
                  i === dayListSize - 1 && 7 - nextMonthCount - 1 < idx
                "
                >{{ day[1] }}</span
              >
              <span class="date" v-else>
                <router-link
                  :to="
                    '/calendar/detail/' +
                    searchParam.searchYear +
                    '/' +
                    searchParam.searchMonth +
                    '/' +
                    day[1]
                  "
                >
...
export default {
  ...
  computed: {
    calendarDayList() {
      ...
      console.log(
        this.searchParam.searchYear + "/" + this.searchParam.searchMonth
      );

      const beforeMonth = new Date(
        this.searchParam.searchYear,
        this.searchParam.searchMonth - 1,
        0
      );
